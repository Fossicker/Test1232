...
function Get-AufbauTabelle {
    # Wertet die Tabelle "Aufbau Anwendungsserver" aus. Sie ist in Bloecke
    # gegliedert:
    #     Appliance                 <- Kategoriezeile
    #     <Datenzeile>              <- gehoert zu Appliance
    #     <Datenzeile>
    #     Betriebssystem            <- naechste Kategoriezeile
    #     ...
    ...
    # Liefert Found, Spalten (Ueberschriften) und Rows (Aufbau + Werte).
    param([Parameter(Mandatory)]$Tables)
    ...
    function Resolve-Kategorie {
        ...
        if (-not $key) { return $null }
        foreach ($kat in $AufbauKategorien) {
            $katKey = Get-NormalizedKey $kat
            if (-not $katKey) { continue }
            if ($key -eq $katKey -or $key.StartsWith($katKey) -or $katKey.StartsWith($key)) { return $kat }
        }
        return $null
    }
    ...
    foreach ($table in $Tables) {
        ...
        for ($r = 0; $r -lt $table.Count; $r++) {
            ...
            if (Resolve-Kategorie $table[$r][0]) { $katZeilen.Add($r) | Out-Null }
        }
        ...
        Write-Verbose "  Aufbau-Tabelle erkannt: $($katZeilen.Count) Kategorien, Offset $offset"
...
foreach ($page in $pages) {
    ...
    $aufbau = @()
    if ($ExportAufbau) {
        $aufbauResult = Get-AufbauTabelle -Tables $tables
...
# --- Ausgabe Aufbau Anwendungsserver: eine Zeile je Eintrag ----------------

$aufbauRows = New-Object System.Collections.Generic.List[object]
$aufbauPath = $null

if ($ExportAufbau) {

    # Spaltennamen eindeutig machen (Excel mag keine doppelten Kopfzeilen)
    $spaltenNamen = New-Object System.Collections.Generic.List[string]
    foreach ($name in $aufbauCols) {
        $kandidat = $name
        $z = 2
        while ($spaltenNamen.Contains($kandidat)) { $kandidat = "$name$z"; $z++ }
        $spaltenNamen.Add($kandidat) | Out-Null
    }

    foreach ($d in $pageData) {
        foreach ($a in $d.Aufbau) {
            $record = [ordered]@{
                Seitenname = $d.Seitentitel
                Verfahren  = $d.Verfahren
                Nr         = $a.Nr
            ...
            for ($i = 0; $i -lt $spaltenNamen.Count; $i++) {
                $wert = ''
                if ($a.Werte.Count -gt $i) { $wert = $a.Werte[$i] }
                $record[$spaltenNamen[$i]] = $wert
            }
            if ($IncludeSeitenIdUndUrl) {
                $record['SeitenId'] = $d.SeitenId
                $record['Url']      = $d.Url
            }
            $aufbauRows.Add([pscustomobject]$record) | Out-Null
        }
    }
...
if ($aufbauPath) {
    ...
    Write-Host "Wertespalten:     $($spaltenNamen -join ', ')" -ForegroundColor DarkGray
}
