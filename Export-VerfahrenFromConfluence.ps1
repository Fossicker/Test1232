<#
    ...
    Liest alle (Unter-)Seiten einer Confluence-Data-Center-Seite aus und exportiert:
      ...
      2. die Matrix-Tabelle "Anwendungsebene | IST | SOLL | Anzahl IST-Server"
         mit einer Zeile je Anwendungsebene
...
function Get-AnwendungsebenenMatrix {
    ...
    foreach ($table in $Tables) {
        ...
        for ($r = $headerIndex + 1; $r -lt $table.Count; $r++) {
            ...
            $ebene = $row[0]
            if ([string]::IsNullOrWhiteSpace($ebene)) { continue }

            $code = Get-EbenenCode $ebene
            if (-not $code) { continue }

            $ist    = Get-CellAt -Row $row -Index $colIst
            $soll   = Get-CellAt -Row $row -Index $colSoll
            $anzahl = Get-CellAt -Row $row -Index $colAnzahl

            $result.Add([pscustomobject]@{
                Ebene  = $ebene
                Code   = $code
                IST    = (ConvertTo-JaNein $ist)
                SOLL   = (ConvertTo-JaNein $soll)
                Anzahl = $anzahl.Trim()
            }) | Out-Null
        }

        break   # erste passende Tabelle genuegt
    }
...
foreach ($page in $pages) {
    ...
    $matrix    = Get-AnwendungsebenenMatrix -Tables $tables
    $matrixMap = @{}
